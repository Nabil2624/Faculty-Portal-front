...
export default function ParticipationInJournals() {
  ...
  // Dummy data
  const journalParticipations = [
    {
      title: "مجلة إدارة الأعمال",
      website: "http://www.jma.org",
      type: "مراجع علمي",
    },
    {
      title: "المجلة الدولية للسياحة",
      website: "http://www.ictj.net",
      type: "عضو هيئة تحرير",
    },
  ];

  const itemsPerPage = 9;
  const totalPages = Math.ceil(journalParticipations.length / itemsPerPage);
  const paginatedData = journalParticipations.slice(
  ...
  return (
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <div className="rounded-lg shadow-none p-0 w-[480px] relative">
        <AddJournalForm onCancel={() => setShowAddForm(false)} />
      </div>
    </div>
  )}
