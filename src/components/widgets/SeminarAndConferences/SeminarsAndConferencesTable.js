...
import { downloadSeminarAttachment } from "../../../services/seminarsAndConferences.service";

// --- 1. مساعدات التنسيق والروابط ---
...
// وظيفة لضمان أن الرابط يفتح خارج الـ localhost
const ensureExternalLink = (url) => {
  if (!url) return "#";
  // إذا كان الرابط لا يبدأ بـ http أو https، نقوم بإضافة https:// تلقائياً
  return url.startsWith("http") ? url : `https://${url}`;
};

// --- 2. مكون الهيدر (البحث والإضافة) ---
...
// --- 3. مكون الفوتر (التنقل بين الصفحات) ---
...
// --- 4. المكون الرئيسي ---
const SeminarsAndConferencesTable = ({
  data = [],
  onEdit,
  onDelete,
  onAdd,
  ...
  const { t: localT, i18n } = useTranslation("SeminarsAndConferences");
  const t = propsT && typeof propsT === "function" ? propsT : localT;
  ...
  const handleDownload = async (item, attachment) => {
    try {
      const response = await downloadSeminarAttachment(item.id, attachment.id);
          ...
          {selectedItem ? (
            <div className="p-10 md:p-14 mx-auto w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                <DetailRow
                  icon={
                    <FileText style={{ width: ICON_SM, height: ICON_SM }} />
                  }
                  label={t("type")}
                  value={t(selectedItem.type)}
                  color={THEME_COLOR}
                  {...{ FS_XS, FS_SM, LH_TIGHT }}
                />
                <DetailRow
                  icon={<Globe style={{ width: ICON_SM, height: ICON_SM }} />}
                  label={t("scope")}
                  value={t(selectedItem.localOrInternational?.toLowerCase())}
                  color={THEME_COLOR}
                  {...{ FS_XS, FS_SM, LH_TIGHT }}
                />
                <DetailRow
                  icon={<Users style={{ width: ICON_SM, height: ICON_SM }} />}
                  label={t("participationRole")}
                  value={
                    selectedItem.roleOfParticipation
                      ? isArabic
                        ? selectedItem.roleOfParticipation.valueAr
                        : selectedItem.roleOfParticipation.valueEn
                      : "-"
                  }
                  color={THEME_COLOR}
                  {...{ FS_XS, FS_SM, LH_TIGHT }}
                />
                <DetailRow
                  icon={<MapPin style={{ width: ICON_SM, height: ICON_SM }} />}
                  label={t("location")}
                ...
                <DetailRow
                  ...
                  label={t("organizingAuthority")}
                  value={selectedItem.organizingAuthority}
                  color={THEME_COLOR}
                  {...{ FS_XS, FS_SM, LH_TIGHT }}
                />

                {/* --- تعديل رابط الموقع لفتحه خارج اللوكال هوست --- */}
                <DetailRow
                  icon={
                    <LinkIcon style={{ width: ICON_SM, height: ICON_SM }} />
                  }
                  label={t("website")}
                  value={
                    selectedItem.website ? (
                      <a
                        href={ensureExternalLink(selectedItem.website)}
                        target="_blank"
                  ...
                  <div className="col-span-full border p-4 rounded-xl bg-amber-50/20">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {selectedItem.attachments.map((a) => (
                        <button
                          key={a.id}
                          onClick={() => handleDownload(selectedItem, a)}
                ...
                <div
                  className="col-span-full mt-4 bg-gray-50 rounded-2xl border-s-4 shadow-sm"
                  style={{
                ...
                >
                  ...
                  >
                    {selectedItem.notes || t("noDescriptionAdded")}
                  </p>
                </div>
              </div>
            </div>
          ) : (
            <div className="text-center opacity-10 select-none h-full flex flex-col justify-center items-center">
              <Globe
                style={{ width: ICON_LG, height: ICON_LG }}
                className="mb-8"
              />
              <p className="font-black uppercase" style={{ fontSize: FS_LG }}>
                {t("selectSeminarToPreview")}
              </p>
            </div>
          )}
        </div>
...
export default memo(SeminarsAndConferencesTable);
