...
const GeneralExperiencesTable = ({
  ...
  // 🔥 الحل البرمجي: مزامنة العنصر المختار مع البيانات المحدثة
  useEffect(() => {
    if (selectedItem) {
      // البحث عن العنصر الحالي في مصفوفة البيانات الجديدة
      const exists = data.find((item) => item.id === selectedItem.id);

      if (!exists) {
        // إذا لم يعد موجوداً (تم حذفه)، نغلق عرض التفاصيل
        setSelectedItem(null);
      } else {
        // إذا كان موجوداً، نحدث البيانات (في حال تم التعديل عليها)
          ...
          {selectedItem ? (
            <div className="p-10 md:p-14 mx-auto w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                ...
                />
                <DetailRow
                  icon={<Clock style={{ width: ICON_SM, height: ICON_SM }} />}
                  label={t("endDate")}
            ...
            <div className="text-center opacity-10 select-none">
              ...
              <p className="font-black uppercase" style={{ fontSize: FS_LG }}>
                {t("selectItemToPreview") || "Select an item to preview"}
              </p>
            </div>
          )}
        </div>
...
export default memo(GeneralExperiencesTable);
