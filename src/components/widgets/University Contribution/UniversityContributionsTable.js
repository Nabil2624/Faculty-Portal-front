...
function formatDate(dateString, i18n, t) {
  ...
  // التأكد من قلب لغة التاريخ بناءً على لغة i18next الحالية
...
// --- 4. المكون الرئيسي ---
const CommunityServiceContributionsTable = ({
  data = [],
  onEdit,
  onDelete,
  onAdd,
  ...
  const FS_XS = "clamp(11px, 0.8vw, 14px)",
    FS_SM = "clamp(13px, 1vw, 20px)",
    FS_BASE = "clamp(15px, 1.4vw, 25px)",
    FS_LG = "clamp(20px, 1.8vw, 35px)";
  const ICON_SM = "clamp(18px, 1.4vw, 26px)",
    ICON_MD = "clamp(22px, 1.8vw, 34px)",
    ICON_LG = "clamp(50px, 5vw, 140px)";
  const LH_TIGHT = "clamp(1.2, 1.4vw, 1.3)",
    LH_NORMAL = "clamp(1.4, 1.6vw, 1.7)";
            ...
            data.map((item) => (
              ...
              >
                {selectedItem?.id === item.id && (
                  <div className="absolute inset-y-0 start-0 w-1" style={{ backgroundColor: THEME_COLOR }} />
                )}
                <h4
                ...
                <div className="flex justify-between items-center mt-3 opacity-70 font-semibold" style={{ fontSize: FS_XS }}>
                  <span className="truncate flex-1 text-[#b38e19]">
                    {isArabic ? item.typeOfContribution?.valueAr : item.typeOfContribution?.valueEn}
                  </span>
                  <span className="shrink-0 bg-gray-100 px-3 py-1 rounded italic">
                    {formatDate(item.dateOfContribution, i18n, t)}
                  </span>
                </div>
              </div>
            ))
        ...
        <div className={`flex-1 overflow-y-auto custom-gold-scrollbar transition-all duration-300 ${!selectedItem ? "hidden md:flex items-center justify-center bg-gray-50/20" : "block"}`}>
          {selectedItem ? (
            <div className="p-10 md:p-14 mx-auto w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                <DetailRow
                  ...
                  label={t("dateOfContribution")}
                  value={formatDate(selectedItem.dateOfContribution, i18n, t)}
                  color={THEME_COLOR}
                  FS_SM={FS_SM}
                  LH_TIGHT={LH_TIGHT}
                />
                <DetailRow
                  icon={<FileText style={{ width: ICON_SM, height: ICON_SM }} />}
                  label={t("typeOfContribution") || t("typeLabel")}
                  value={isArabic ? selectedItem.typeOfContribution?.valueAr : selectedItem.typeOfContribution?.valueEn}
                  color={THEME_COLOR}
                  FS_SM={FS_SM}
                  LH_TIGHT={LH_TIGHT}
                />
            ...
            <div className="text-center opacity-10 select-none">
              <HeartHandshake style={{ width: ICON_LG, height: ICON_LG }} className="mx-auto mb-8" />
              <p className="font-black uppercase" style={{ fontSize: FS_LG }}>
                {t("selectItem") || t("selectItemToPreview")}
              </p>
            </div>
          )}
        </div>
...
export default memo(CommunityServiceContributionsTable);