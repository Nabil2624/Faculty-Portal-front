...
function formatDate(dateString, i18n, t) {
  if (!dateString) return t("projects.present");
  ...
  <div className="p-3 border-b bg-gray-50 flex items-center justify-between gap-4 shrink-0">
    ...
    <div className="flex items-center gap-2">
      ...
      <button onClick={onAdd} style={{ backgroundColor: THEME_COLOR, fontSize: FS_SM }} className="flex items-center gap-2 px-6 py-3 text-white rounded-lg hover:opacity-90 transition-all shadow-sm font-bold shrink-0 active:scale-95">
        ...
        <span className="hidden md:inline">{t("projects.addNew")}</span>
      </button>
    </div>
  </div>
));
    ...
    <div dir={isArabic ? "rtl" : "ltr"} className="flex flex-col h-[73vh] 3xl:h-[83vh] bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden text-gray-700">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className={`border-e overflow-y-auto custom-gold-scrollbar bg-white shrink-0 transition-all duration-300 ${selectedItem ? "hidden md:block" : "block"}`} style={{ width: selectedItem ? MASTER_WIDTH : "100%" }}>
          {data.length === 0 ? (
            <div className="p-10 text-center text-gray-400 italic" style={{ fontSize: FS_SM }}>{t("projects.noDataAvailable")}</div>
          ) : (
            data.map((item) => (
              <div
                key={item.id}
              ...
              >
                ...
                <div className="flex justify-between items-center mt-3 opacity-70 font-semibold" style={{ fontSize: FS_XS }}>
                  <span className="truncate flex-1">{isArabic ? item.typeOfProject?.valueAr : item.typeOfProject?.valueEn}</span>
          ...
          {selectedItem ? (
            <div className="p-10 md:p-14 mx-auto w-full">
              ...
              <div className="flex flex-col md:flex-row justify-between gap-8 mb-12 border-b pb-10">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-4 mb-4" style={{ color: THEME_COLOR }}>
                    ...
                    <span style={{ fontSize: "clamp(22px, 1.5vw, 30px)" }} className="font-semibold uppercase tracking-widest">{t("projects.documentDetails")}</span>
                  </div>
                  <h3 className="font-semibold text-[#19355a] break-words" style={{ fontSize: FS_LG, lineHeight: LH_TIGHT }}>
                    {selectedItem.nameOfProject}
                  </h3>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                <DetailRow icon={<Globe style={{ width: ICON_SM, height: ICON_SM }} />} label={t("projects.localInternational")} value={selectedItem.localOrInternational === "Local" ? t("projects.local") : t("projects.international")} color={THEME_COLOR} FS_SM={FS_SM} LH_TIGHT={LH_TIGHT} />
                <DetailRow icon={<DollarSign style={{ width: ICON_SM, height: ICON_SM }} />} label={t("projects.fundingAuthority")} value={selectedItem.financingAuthority} color={THEME_COLOR} FS_SM={FS_SM} LH_TIGHT={LH_TIGHT} />
                <DetailRow icon={<User style={{ width: ICON_SM, height: ICON_SM }} />} label={t("projects.participationRole")} value={isArabic ? selectedItem.participationRole?.valueAr : selectedItem.participationRole?.valueEn} color={THEME_COLOR} FS_SM={FS_SM} LH_TIGHT={LH_TIGHT} />
                <DetailRow icon={<Calendar style={{ width: ICON_SM, height: ICON_SM }} />} label={t("projects.startDate")} value={formatDate(selectedItem.startDate, i18n, t)} color={THEME_COLOR} FS_SM={FS_SM} LH_TIGHT={LH_TIGHT} />
                <DetailRow icon={<Calendar style={{ width: ICON_SM, height: ICON_SM }} />} label={t("projects.endDate")} value={formatDate(selectedItem.endDate, i18n, t)} color={THEME_COLOR} FS_SM={FS_SM} LH_TIGHT={LH_TIGHT} />
                ...
                <div className="col-span-full mt-8 bg-gray-50 rounded-2xl border-s-4 shadow-sm" style={{ borderInlineStartColor: THEME_COLOR, padding: "clamp(1rem, 1.2vw, 2rem)" }}>
                  <label className="block uppercase mb-3 tracking-widest font-black" style={{ fontSize: FS_SM }}>{t("projects.description")}</label>
                  <p className="text-gray-700 italic break-words whitespace-pre-wrap text-justify" style={{ fontSize: FS_SM, lineHeight: LH_NORMAL }}>
                    {selectedItem.description || t("projects.noDescriptionAdded")}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center text-center p-10 select-none">
              <Briefcase style={{ width: ICON_LG, height: ICON_LG }} className="opacity-10 mb-8" />
              <p className="font-black text-gray-300 uppercase tracking-widest" style={{ fontSize: FS_LG }}>{t("projects.selectItemToPreview")}</p>
            </div>
          )}
        </div>
      </div>
