...
const TopResearchersCard = ({ title = "أفضل الباحثين بالكلية" }) => {
  const [selectedCollegeId, setSelectedCollegeId] = useState("");
  const [open, setOpen] = useState(false);
  const dropdownRef = useRef(null);

  const { faculties, loading: facultiesLoading } = useFaculties();
  const { data: researchers, loading, fetchTopResearchers } = useTopResearchers();
        ...
        <div className="relative mb-4" ref={dropdownRef}>
          {facultiesLoading ? (
            ...
            <div
              ...
              // ✅ البوردر تم نقله لجهة اليمين (Start)
              style={{ borderRight: `4px solid ${gold}` }} 
            >
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                <span className="text-[10px] font-bold text-[#19355A]">
                  {faculties.find((c) => c.id == selectedCollegeId)?.name || "اختر الكلية"}
                </span>
              </div>
              <ChevronDown size={14} className={`text-[#B38E19] transition-transform duration-300 ${open ? "rotate-180" : ""}`} />
            </div>
          )}
              ...
              {faculties.map((col) => (
                <div
                  ...
                  className={`p-3 text-[11px] font-bold cursor-pointer hover:bg-gray-50 transition-colors ${
                    selectedCollegeId == col.id ? "bg-gray-100 text-[#B38E19]" : "text-[#19355A]"
                  }`}
                >
                  {col.name}
                </div>
