  ...
  <div className="p-3 border-b bg-gray-50 flex items-center justify-between gap-4 shrink-0">
    ...
    <div className="flex items-center gap-2">
      ...
      <button onClick={onAdd} style={{ backgroundColor: THEME_COLOR, fontSize: FS_SM }} className="flex items-center gap-2 px-6 py-3 text-white rounded-lg hover:opacity-90 transition-all shadow-sm font-bold shrink-0 active:scale-95">
        ...
        <span className="hidden md:inline">{t("journals.addNew")}</span>
      </button>
    </div>
  </div>
));
...
// --- 3. المكون الرئيسي ---
const JournalMasterDetailTable = ({
  data = [],
  onEdit,
  onDelete,
  onAdd,
  ...
  return (
    <div dir={isArabic ? "rtl" : "ltr"} className="flex flex-col h-[73vh] 3xl:h-[83vh] bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden text-gray-700">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        >
          {data.length === 0 ? (
            <div className="p-10 text-center text-gray-400 italic" style={{ fontSize: FS_SM }}>{t("journals.noDataAvailable")}</div>
          ) : (
            data.map((item) => (
              <div
                key={item.id}
              ...
              >
                ...
                <div className="flex justify-between items-center mt-3 opacity-70 font-semibold" style={{ fontSize: FS_XS }}>
                  <span className="truncate flex-1">{isArabic ? item.typeOfParticipation?.valueAr : item.typeOfParticipation?.valueEn}</span>
                  {item.websiteOfMagazine && <Globe size={14} className="text-[#b38e19]" />}
                </div>
              </div>
            ))
          )}
        ...
        <div className={`flex-1 overflow-y-auto custom-gold-scrollbar transition-all duration-300 ${!selectedItem ? "hidden md:flex flex-col items-center justify-center bg-gray-50/20" : "block"}`}>
          {selectedItem ? (
            <div className="p-10 md:p-14 mx-auto w-full">
              ...
              <div className="flex flex-col md:flex-row justify-between gap-8 mb-12 border-b pb-10">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-4 mb-4" style={{ color: THEME_COLOR }}>
                    ...
                    <span style={{ fontSize: "clamp(22px, 1.5vw, 30px)" }} className="font-semibold uppercase tracking-widest">{t("journals.documentDetails")}</span>
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                <DetailRow 
                  ...
                  label={t("journals.participationType")} 
                  value={isArabic ? selectedItem.typeOfParticipation?.valueAr : selectedItem.typeOfParticipation?.valueEn} 
                  color={THEME_COLOR} FS_SM={FS_SM} LH_TIGHT={LH_TIGHT} 
                />
                
                <div className="flex items-start gap-5 min-w-0">
                  <div className="flex items-center justify-center rounded-xl bg-white border shadow-sm shrink-0" style={{ color: THEME_COLOR, width: "clamp(42px,3.5vw,52px)", height: "clamp(42px,3.5vw,52px)" }}>
                    <Globe style={{ width: ICON_SM, height: ICON_SM }} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="uppercase mb-2 font-semibold opacity-60" style={{ fontSize: "0.7rem" }}>{t("journals.journalWebsite")}</p>
                    {selectedItem.websiteOfMagazine ? (
                      <button 
                        onClick={() => handleOpenWebsite(selectedItem.websiteOfMagazine)}
                        className="font-semibold text-[#b38e19] underline break-all text-start" 
                        style={{ fontSize: FS_SM, lineHeight: LH_TIGHT }}
                      >
                        {selectedItem.websiteOfMagazine}
                      </button>
                    ) : (
                      <p className="font-semibold text-gray-400" style={{ fontSize: FS_SM }}>---</p>
                    )}
                  </div>
                </div>
                ...
                <div className="col-span-full mt-8 bg-gray-50 rounded-2xl border-s-4 shadow-sm" style={{ borderInlineStartColor: THEME_COLOR, padding: "clamp(1rem, 1.2vw, 2rem)" }}>
                  <label className="uppercase mb-3 tracking-widest font-black flex items-center gap-2" style={{ fontSize: FS_SM }}>
                    <FileText size={18} /> {t("journals.notes")}
                  </label>
                  <p className="text-gray-700 italic break-words whitespace-pre-wrap text-justify" style={{ fontSize: FS_SM, lineHeight: LH_NORMAL }}>
                    {selectedItem.notes || t("journals.noNotes")}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center text-center p-10 select-none">
              <BookOpen style={{ width: ICON_LG, height: ICON_LG }} className="opacity-10 mb-8" />
              <p className="font-black text-gray-300 uppercase tracking-widest" style={{ fontSize: FS_LG }}>
                {t("journals.selectItemToPreview")}
              </p>
            </div>
          )}
        </div>
...
export default memo(JournalMasterDetailTable);