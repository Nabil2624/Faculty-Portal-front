...
export default function EditCommitteeAssociation() {
  ...
  useEffect(() => {
    if (existingData) {
      setCommittee(existingData.committee || "");
      setTypeValue(existingData.type || "");
      setDegreeValue(existingData.degree || "");
  ...
  const handleSave = () => {
    // هتعمل API Update هنا لو عندك
    navigate("/committees-associations");
  };
